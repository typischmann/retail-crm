...
BEGIN
	IF NEW.ENCRYPTED_USER_PASSWORD IS NOT NULL THEN
		NEW.ENCRYPTED_USER_PASSWORD := crypt(NEW.ENCRYPTED_USER_PASSWORD, gen_salt('md5'));
	END IF;
	NEW.DELTA_TS := current_timestamp;
	RETURN NEW;
END; 
...
BEGIN
	RETURN (SELECT USER_PASSWORD = crypt(sys_user_password, ENCRYPTED_USER_PASSWORD) FROM SYS_USERS WHERE USER_NAME = sys_user_name);
END;
